onmessage = function (message) {
    ...
    let root_position = data.position;
    let use_old_mcn = data.use_old_mcn;
    ...
    for (let y = 0; y < height; y++) {
        ...
        for (let i = 0; i < temp_col.length; i++) {
            ...
            for (let j = 0; j < temp_row.length; j++) {
                ...
                let yx = root_position[1];
                let zx = root_position[2] + y * render_level + i;

                let cmd = "setblock " + xx + " " + yx + " " + zx + " " + (use_old_mcn ? blocks[id].mcn : blocks[id].new_mcn);
                mcfunc += cmd + "\n";
